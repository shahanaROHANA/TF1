...
const RatingsFeedback = () => {
  ...
  const fetchRatings = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/seller/dashboard/ratings?page=${page}`, {
        headers: {
      ...
      if (response.ok) {
        ...
        setStats(data.stats || { totalRatings: 0, averageRating: 0 });
      } else {
        const error = await response.json();
        setMessage(error.message || 'Failed to load ratings');
      }
    } catch (error) {
      console.error('Error fetching ratings:', error);
      setMessage('Error loading ratings');
    } finally {
      setLoading(false);
    }
  };
