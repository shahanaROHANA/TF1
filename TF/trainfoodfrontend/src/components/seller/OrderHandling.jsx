...
const OrderHandling = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/seller/dashboard/orders?${queryParams}`, {
        headers: {
  ...
  const updateItemStatus = async (orderId, itemId, newStatus, note = '') => {
    ...
    try {
      const token = localStorage.getItem('sellerToken') || localStorage.getItem('token');
      const response = await fetch(`${import.meta.env.VITE_API_URL}/seller/dashboard/item/update`, {
        method: 'POST',
        headers: {
      ...
      setMessage('Error updating item status. Please try again.');
    } finally {
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    ...
    try {
      const token = localStorage.getItem('sellerToken') || localStorage.getItem('token');
      const response = await fetch(`${import.meta.env.VITE_API_URL}/orders/${orderId}/status-tracking`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
